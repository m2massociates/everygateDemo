...
//    	    	break;
        ...
        for (var i = 0; i < input.length; i++) {
            ...
            if (0xD800 <= charCode && charCode <= 0xDBFF) {
                ...
                if (isNaN(lowCharCode)) {
                    console.log(new Error(format(ERROR.MALFORMED_UNICODE, [charCode, lowCharCode])));
                }
    ...
    function parseUTF8(input, offset, length) {
        ...
        while (pos < offset + length) {
            ...
            if (byte1 < 128)
                ...
                if (byte2 < 0)
                    console.log(new Error(format(ERROR.MALFORMED_UTF, [byte1.toString(16), byte2.toString(16), ""])));
                if (byte1 < 0xE0)             // 2 byte character
                    utf16 = 64 * (byte1 - 0xC0) + byte2;
                else {
                    var byte3 = input[pos++] - 128;
                    if (byte3 < 0)
                        console.log(new Error(format(ERROR.MALFORMED_UTF, [byte1.toString(16), byte2.toString(16), byte3.toString(16)])));
                    if (byte1 < 0xF0)        // 3 byte character
                        utf16 = 4096 * (byte1 - 0xE0) + 64 * byte2 + byte3;
                    else {
                        var byte4 = input[pos++] - 128;
                        if (byte4 < 0)
                            console.log(new Error(format(ERROR.MALFORMED_UTF, [byte1.toString(16), byte2.toString(16), byte3.toString(16), byte4.toString(16)])));
    ...
    var Client = function (host, port, clientId) {
        ...
        if (typeof clientId !== "string" || clientIdLength < 1 | clientIdLength > 53)
